...
test('refreshing feed preserves product ids and ai job references', function () {
...
XML;
    ...
    expect($product1)->not->toBeNull('Product 1 should still exist');
    expect($product2)->not->toBeNull('Product 2 should still exist');

    // Assert: Product data should be updated
    expect($product1->title)->toEqual('Product One (Updated)');
    expect($product2->title)->toEqual('Product Two (Updated)');
    ...
    $aiJob->refresh();
    expect($aiJob->product_id)->toEqual($originalProduct1Id, 'AI job should still reference the same product ID');
    expect($aiJob->product)->not->toBeNull('AI job should be able to load the product relationship');
    expect($aiJob->product->title)->toEqual('Product One (Updated)');

    // Assert: Total product count should be 3
    expect(Product::where('product_feed_id', $feed->id)->count())->toEqual(3);
});